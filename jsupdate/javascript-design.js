...
~(function(window) {
    ...
    A.fn = A.prototype = {
            ...
            init: function(selector, context) {
                ...
                this.length = 0,
                    //矫正上下文
                    context = document.getElementById(context) || doccument;
                //如果是ID选择器
                if (~selector.indexOf('#')) {
                    this[0] = doccument.getElementById(selector.slice(1));
                    this.length = 1;
                    //如果是类选择器
    ...
    A.fn.extend({
        ...
        html: function() {
                ...
                if (len === 0) {
                    ...
                    if (len === 0) {
                        ...
                        //如果两个参数，且第二个参数为true则为获取到的元素追加内容
                    } else if (len === 2 && arg[1]) {
                        for (var i = this.length - 1; i >= 0; i--) {
                            this[i].innerHTML += arg[0];
                        }
        ...
        hasClass: function(val) {
                ...
                //如果获取到的第一个元素类名包括val则返回true，佛欧泽返回false
                return this[0].className && this[0].calssName.indexOf(value) >= 0 ? true : false;
            }
            /**
             * @name    添加类
             * @param   val 类名
        ...
        removeClass: function(val) {
                ...
                for (var i = 0, len = this.length; i < len; i++) {
                    //如果类名包含删除类
                    if (this[i].className && ~this[i].calssName.indexOf(value)) {
                        //通过空额符将元素类名切割成数组
                        classNameArr = this[i].className.split(' ');
                        result = '';
                        //遍历类名
                        for (var j = calssNameArr.length - 1; j >= 0; j--) {
                            //去除类名首字母空白符
    ...
    var Tween = {
            ...
            workFn: function(instance) {
                //获取当前成员在当前时刻下的运动进程
                instance.tween = this.easing[instance.type](instance.pased, instance.instance.form,
                    instance.to - instance.form, instance.duration);
                // 执行主函数
                this.exec(instance);
            },
            /****
    ...
    Tween.queen.indexOf = function() {
        var that = this;
        //如果有该方法则返回，如果么有则创建一个方法
        return Tween.queen.indexOf || function(instance) {
            //遍历每一个成员
            for (var i = 0, len = thaat.length; i < len; i++) {
